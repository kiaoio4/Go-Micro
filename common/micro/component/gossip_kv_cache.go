...
type GossipKVCacheComponent struct {
	...
	items    map[string]string //key-value e.g. id-cluster
}
...
func (c *GossipKVCacheComponent) SetupHandler(root echoswagger.ApiRoot, base string) error {
	...
	g.POST("", c.addHandler).
		...
		SetOperationId("add").
		SetSummary("add a key-value pair, if value is not set, kv will use consider is a id-cluster item and use its cluster name as value")
	g.GET("", c.getHandler).
	...
	g.GET("/demo1", c.GoMicroHandlerWrapper(basicConf.Service, c.demoHandler, false)).
		AddParamQuery("", "id", "id", true).
...
func (c *GossipKVCacheComponent) demoHandler(ctx echo.Context) error {
	sensorid := ctx.QueryParam("id")
	if sensorid != "" {
		c.l().Infow("the real response", "id", sensorid, "privateip", c.meta.PrivateIP, "globalip", c.meta.GlobalIP)
	...
	c.l().Infof("the real response", "id", sensorid, "privateip", c.meta.PrivateIP, "globalip", c.meta.GlobalIP)
	data := []string{}
...
// GoMicroHandlerWrapper return a echo.HandlerFunc that will forward/redirect request to the instance which serve with the sensorid if need
// v2 version, if permitSelfHandlerFunc is true, use self handler func when no sensorID
func (c *GossipKVCacheComponent) GoMicroHandlerWrapper(selfServiceName string, f echo.HandlerFunc, permitSelfHandlerFunc bool) echo.HandlerFunc {
